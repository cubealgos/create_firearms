...
| `tools/models.py` | Cuboid weapon and attachment-part item models (WEAPON-REQ-016, `decisions/DEC-018-art-direction.md`). | `ramp_for(material)`, `class_display(class_name)`, `build_atlas(boxes)`, `weapon_model_json(weapon_id, boxes, size, uv_map, class_name)`, `part_model_json(slot, name, class_name, boxes, anchor, size, uv_map)`, `generate_weapon_item_json()`, `render_sheet(path)`, `main()` |
...
| `tools/test_models.py` | `tools/models.py`, exercised the way the generator is actually used: run it, diff the tree against a second run (idempotency: no random or wall-clock input anywhere in the generator), and check every box face's uv rectangle the atlas packer hands out actually lies inside that atlas (`FA-22`). |  |
