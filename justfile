...
vault_spec := env("FA_VAULT_SPEC", main_checkout / ".." / "heimathafen" / "vault" / "projects" / "create_firearms" / "spec")
...
release:
    ...
    cp "build/libs/create_firearms-$version.jar" dist/
    (cd dist && shasum -a 256 "create_firearms-$version.jar" > "create_firearms-$version.jar.sha256")
